...
const CategoryItem = ({ title, selected, onClick }: props) => {
  ...
  return (
    ...
    >
      ...
      {title != "Following" && (
        <button className="invisible absolute right-[-5%] top-[-25%] rounded-full bg-red-400 px-2 text-[8px] text-white hover:bg-red-500 group-hover/categoryItem:visible">
          x
        </button>
      )}
    </div>
